...
const Subject = (props) => {
    ...
    const isFormValid = props.elementEdit
        ? isDifferent
    ...
    useEffect(() => {
        if (props.elementEdit) {
            const verifyDataToUpdate = verifyIfDataIsDiferente(
                [
                    {
                        ancientElement: dataCourse?.subjectName?.name,
                        recentElement: nameSubject,
                        nameField: "name",
                    },
                    {
                        ancientElement: dataCourse?.teacherId,
                        recentElement: teacherSubject,
                        nameField: "teacherId",
                    },
                ],
                setIsDifferent,
    ...
    const handleAction = async (event) => {
        ...
        try {
            ...
            let message;
            if (props.elementEdit) {
                const response = await updateSubjects(
                    accessToken,
                    setAccessToken,
                    refreshToken,
                    setRefreshToken,
                ...
                const response = await postSubjects(
                    ...
                    setRefreshToken,
                    props.academicId,
                    { name: nameSubject, teacherId: teacherSubject }
                );
                message = response.message;
            }
    ...
    return (
        <>
            <form
                className="grid grid-cols-1 gap-4 pt-6 px-4"
            ...
            >
                ...
                >
                    {(teacher) => (
                        <AutocompleteItem key={teacher.value}>
                            {teacher.label}
                        </AutocompleteItem>
                    )}
                </Autocomplete>
