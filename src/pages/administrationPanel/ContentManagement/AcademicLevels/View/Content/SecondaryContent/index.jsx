...
const SecondayContent = () => {
    ...
    useEffect(() => {
        const callAPI = async () => {
            try{
                ...
                if (isSubject) {
                    const responseTeacher = await getUsers(accessToken, setAccessToken, refreshToken, setRefreshToken, {rol: 'docente'});
                    setTeachers(responseTeacher.elements);
                    const params = {
                        teacher: valueFilter.Profesor?.size > 0 ? valueFilter?.currentKey : null,
                        search: search !== '' ? search : undefined,
                    };
                    const response = await getSubjects(accessToken, setAccessToken, refreshToken, setRefreshToken, academicId, params);
                    setGetData({
                    ...
                    const response = await getCourses(accessToken, setAccessToken, refreshToken, setRefreshToken, academicId, params);
                    setGetData({
    ...
    const handleDelete = async (ids) => {
        try {
            ...
            const elementToDeleteFuntion = getData.elementName === 'asignatura' ? deleteSubjects : deleteCourse
            for (const element of elementToDelete) {
                await elementToDeleteFuntion(accessToken, setAccessToken, refreshToken, setRefreshToken, element);
            }
