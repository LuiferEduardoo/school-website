...
const dataForTheAPI = (states) => {
    ...
    if(states.authors.length > 0){
        data.authors = states.authors.map(m => m.id).join(',')
    }
...
const verifyDataToUpdate = (states, setIsDifferent) => verifyIfDataIsDiferente([
    ...
    {ancientElement: states.ancientAuthors, recentElement: states.recentAuthors, nameFieldOne: 'idsNewAuthors', elementType: 'membersOrAuthor', action: 'update', haveCoordinator: false},
    {ancientElement: '', recentElement: formart(states.idEliminateAuthors.map(id => id.id)), nameField: 'idsEliminateAuthors'},
