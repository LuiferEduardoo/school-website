...
const contentManagementOptions = [
  { path: 'content-management/academic-levels', label: 'Niveles académicos', isProtected: true },
  { path: 'content-management/news', label: 'Noticias', isProtected: true },
  { path: 'content-management/institutional-projects', label: 'Proyectos institucionales', isProtected: false },
      ...
      <>
        <ul className='flex flex-col gap-3 pl-11 pt-2'>
          {contentManagementOptions.map((option, index) => (
            !option.isProtected || (option.isProtected && superAdmin.includes(rolUser)) ? (
              <li key={index}>
